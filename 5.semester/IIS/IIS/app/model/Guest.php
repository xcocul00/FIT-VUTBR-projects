...
/****************************************************************/
   public function filter($arr, $filters){
	...
	public function getAccomodations($id){
		$accomodations = $this->database->table("Ubytovania")
			->where('Typ_ubytovania','Ubytovanie')
			->where('Host_id_host', $id);
		$result = array();

		foreach ($accomodations as $reserv) {
			$guest = $this->database->table('Host_hotela')->where('Host_id',$id)->fetch();
...
public function createReservation($values, $start_date, $end_date, $guest, $room_info, $all_services, $user){
		$rooms = array();
		$services = array();
		$prize = 0;
...
/****************************************************************/
	public function getReservations($id){
		$reservations = $this->database->table("Ubytovania")
			->where('Typ_ubytovania','Rezervacia')
			->where('Host_id_host',$id);
		$result = array();
