...
*/
	if (q == NULL)  // ak je q == Null potom sa o¹etrí chyba
	{
		queueError(QERR_INIT);
	}
	else 
	{
		for (unsigned int pomocna=0; pomocna < QUEUE_SIZE; pomocna++) // nastavenie prvkov na *
		{
			q->arr[pomocna]= '*';
		}
		q->f_index = 0; // nulovanie indexov 
		q->b_index = 0;
	}
...
*/
	return (index + 1 ) % QUEUE_SIZE;   // vráti index nasledujúceho prvku
...
*/	
	return q-> f_index == q-> b_index;  // vráti nenulovu hodnotu ak je fronta prázdná
...
*/
	return q->f_index == nextIndex(q->b_index);  // vráti nenulovú hodnotu ak je fronta plná
...
*/
	if (queueEmpty(q) != 0)   // ak je fronta prázdna o¹etrí chybu
	{
	...
	{
		*c = q->arr[q->f_index]; // vráti znak na prvom mieste
	}
...
*/
	...
	{
		q->f_index = nextIndex(q->f_index); // pretoèí frontu
	}
...
*/
	...
	else                      // odstráni znak zo zaèiatku fronty a vráti jeho hodnotu
	{
		queueFront(q,c);
		queueRemove(q);
...
*/
	if (queueFull(q) != 0)  // ak je fronta prázdna o¹etrí chybu
	{
		queueError(QERR_UP);
	}
	else
	{
		q->arr[q->b_index] = c;  // vlo¾í znak 
		q->b_index = nextIndex(q->b_index); // posun na ïal¹ie prázdne miesto
	}
