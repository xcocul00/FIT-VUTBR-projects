...
**/
	L->Act=NULL;      //prázdny aktuálny
	L->First=NULL;    // prázdny prvý
...
***/
	...
	while (L->First != NULL) {     // pokiaµ nezru¹ím v¹etky prvky zoznamu
		tElemPtr tmp = L->First;   // doèasný ukazateµ na prvý prvok zoznamu
		L->First = L->First->ptr;  // posun v prvkoch o miesto ïalej
		free (tmp);				   // zru¹í doèasný ukazateµ	 						
	}
	L->Act = NULL;  // nastaví prvok na neaktívny
...
** volá funkci Error().
**/
	tElemPtr tmp;     // nový prvok
	if ((tmp = malloc(sizeof(struct tElem))) == NULL )  // alokácia prvku
	{
		Error();    // o¹etrenie chyby alokácie
		return;     
	}
		tmp->data = val;       // hodnota nového prvku
		tmp->ptr = L->First;   // prvý prvok ukazuje na bývalý prvý
 		L->First = tmp;        // nastavenie prvku ako prvého 
...
**/
	L->Act = L->First; // prvý prvok nastaví na aktívny
...
**/
	if (L-> First == NULL)   // zistí èi je zoznam prázdny
	{
...
**/
	if (L->First == NULL)   // nerobí niè ak je zoznam prázdny
	{
		return;
	}	
	if (L->Act == L->First) // ak je prvý aktívny, deaktivuje ho
	{
		L->Act=NULL;
	}
	tElemPtr tmp = L->First; // doèasný ukazateµ na následujúci prvok
	L->First = L->First->ptr; // prepojenie na ïal¹í prvok
	free(tmp);               // zru¹í starý prvý prvok
...
** nic se nedìje.
**/
	if (L->Act == NULL || L->Act->ptr == NULL)  // pokiaµ je zoznam neaktívny niè nerobí
	{
		return;
	}
	tElemPtr tmp = L->Act->ptr;          // doèasný ukazateµ na následujúci prvok
	L->Act->ptr = L->Act->ptr->ptr;      // prepojenie na ïal¹í prvok
	free (tmp);                          // zru¹í starý prvý prvok
...
**/
	...
	tElemPtr tmp;        // nový prvok
	if ((tmp = malloc(sizeof(struct tElem))) == NULL)  // alokácia nového prvku
	{
		Error();
		return;
	}
	tmp->data = val;           // vlo¾enie hodnoty prvku
	tmp->ptr = L->Act->ptr;    // prepojenie na následujúci prvok
	L->Act->ptr = tmp;         // nový prvok bude za následujúcim aktívnym  
...
**/
	if (L->Act == NULL)     //  pokiaµ nie je zoznam aktívny o¹etrí chybu
	{
		Error();
		return;
	}
	*val = L->Act->data;    // preèíta aktivné dáta a vráti ich
...
**/
	...
	L->Act->data = val;  // vlo¾í hodnotu val ako dáta aktívneho prvku
...
**/
	if (L-> Act == NULL)  // pokiaµ nie je zoznam aktívny nerobí niè
	{
		return;
	}	
	L->Act = L->Act->ptr; // posunie aktivitu na ïal¹í prvok zoznamu
...
**/
	...
	return L->Act != NULL;   // ak je zoznam aktívny vracia nenulovú hodnotu, inak vracia 0
}
